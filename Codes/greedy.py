...
if __name__ == "__main__":
    ...
    for sim_num in range(1,101):
        ...
        for t in range(100):
            ...
            for i in range(3):
                for j in range(3):
                    ...
                    _, reward, _, _ = temp_env.step([i,j])
                    reward = [np.random.normal(i, 0.3*abs(i)) for i in reward]
                    if reward[0] > max_reward:
                        max_reward = reward[0]
                        max_action = [i,j]
            state, reward, done, temp_metric = env.step(max_action)
        ...
        print("Progress:%d/%d"%(sim_num,100))
    directory = "results_greedy/greedy_%.1f_%.1f_%.1f_%.2f_%d_%.1f/"%(arglist.pida,arglist.peda,arglist.pnca,arglist.tl,arglist.nd,arglist.alpha)
