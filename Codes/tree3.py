...
def train_ppo(states,batch_size=0,bplus=0,random_seed=0,last_evaluation_episodes=0,adv='fgs',padv=0,load=False,save=True,root=None,sroot=None,action_list=None):
    ...
    for time_step in range(20):
        ...
        for num in range(50):
            ...
            for t in range(max_ep_len):
                ...
                if t<7:
                    actions = [agents[i].select_action3(env.state,i,action_list[t][i]) for i in range(nagents)]
                else:
        ...
        for n in output:
            ...
            for idx in range(len(n)):
                if idx < 7:
                    temp1 = temp1.next_list[n[idx]]
                    rewards[-1].append(temp1.value)
                else:
                    temp2 = temp2.next_list[n[idx]]
                    rewards[-1].append(temp2.value)
    ...
    for t in range(max_ep_len):        
        ...
        if done:
            break
    return reward_list, metric_list, final_reward

def train_dqn(states,batch_size=0,bplus=0,random_seed=0,last_evaluation_episodes=0,adv='fgs',padv=0,load=False,save=False,root=None,sroot=None,action_list=None):
    ...
    while time_step < max_training_timesteps:
        ...
        for t in range(max_ep_len):                           
            # select action with policy
            if t<7:
                actions = [action_list[t][i] for i in range(nagents)]
                next_state, total_action, done = env.step(actions,states)
                temp1 = temp1.next_list[total_action]
                current_ep_reward += temp1.value
                for i in range(nagents):
                    agents[i].remember(deepcopy(state).flatten(), deepcopy(actions[i]), deepcopy(temp1.value), deepcopy(next_state).flatten(), done)
            else:
                actions = [agents[i].select_action(state, i) for i in range(nagents)]
                next_state, total_action, done = env.step(actions,states)
                temp2 = temp2.next_list[total_action]
                current_ep_reward += temp2.value
                for i in range(nagents):
                    agents[i].remember(deepcopy(state).flatten(), deepcopy(actions[i]), deepcopy(temp2.value), deepcopy(next_state).flatten(), done)
    ...
    for t in range(max_ep_len):                           
        # select action with policy
        actions = [agents[i].select_action(state, i) for i in range(nagents)]
        next_state, reward, done, temp_metric = env.step(actions)
        final_reward += sum(reward)/2
        metric_list += temp_metric
        state = deepcopy(next_state)
        if done:
...
if __name__ == "__main__":
    ...
    ref_lists = pickle.load(open("e2/%s/ref_%d_%s_%.1f.pkl"%(arglist.drl,arglist.um,arglist.adv,arglist.padv),"rb"))
    action_lists = pickle.load(open("e2/%s/a_%d_%s_%.1f.pkl"%(arglist.drl,arglist.um,arglist.adv,arglist.padv),"rb"))
    ...
    for i in range(len(ref_lists)):
        action_list = action_lists[i]
        sroot = pickle.load(open("e2/%s/r_%d_%s_%.1f/r_%d.pkl"%(arglist.drl,arglist.um,arglist.adv,arglist.padv,i),"rb"))
        for j in range(3):
            for k in range(2):
                name = 's%d%d'%(j+7,k)
                states[name] = pickle.load(open('e2/%s/s_%d_%s_%.1f/%d/s%d%d.pkl'%(arglist.drl,arglist.um,arglist.adv,arglist.padv,i,j,k),'rb'))
        #print(list(states.keys()))
        for ref in ref_lists[i]:
            reward_list, metric_list, final_reward = train(states,random_seed=arglist.idx*arglist.step+ref,adv=arglist.adv,padv=arglist.padv,load = True,root=root,sroot=sroot,action_list=action_list)
            rewards_list.append(reward_list)
            metrics_list.append(metric_list)
            final_reward_list.append(final_reward)
    directory = "results/%s_%d_%s_%.1f/"%(arglist.drl,arglist.um,arglist.adv,arglist.padv)
    if not os.path.exists(directory):
        os.makedirs(directory)
    pickle.dump(rewards_list,open(directory+"rewards_list.pkl","wb"))
