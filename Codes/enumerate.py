...
def parse_args():
    ...
    parser.add_argument("--pos", metavar='N', type=int, default=0)
    parser.add_argument("--end", metavar='N', type=int, default=27)
    parser.add_argument("--ty", metavar='attacks/battery/sun/nodes', type=str, default='pida') 
...
if __name__ == "__main__":
    for i in range(7):
        directory = "%d/%s/results%.1f/%d/"%(arglist.um,arglist.ty,dicts[arglist.ty],i+1)
    ...
    for i in range(6):
        ...
        pickle.dump(reward_list,open("%d/%s/results%.1f/%d/rewards_list.pkl"%(arglist.um,arglist.ty,dicts[arglist.ty],i+1),"wb"))
        pickle.dump(state_list,open("%d/%s/results%.1f/%d/state_list.pkl"%(arglist.um,arglist.ty,dicts[arglist.ty],i+1),"wb"))
    print('step 6: done')
    indices = list(range(0,len(env_list)+1,int(len(env_list)/27)))
    #for i in [arglist.pos]:
        #reward_list, _, state_list = multi_train(env_list[indices[i]:indices[i+1]])
        #pickle.dump(reward_list,open("%s/results%.1f/%d/rewards_list_%d.pkl"%(arglist.ty,arglist.ap,7,i),"wb"))
        #pickle.dump(state_list,open("%s/results%.1f/%d/state_list_%d.pkl"%(arglist.ty,arglist.ap,7,i),"wb"))
    #print('done')
        #pickle.dump(env_list,open("results/%d/next_env_list.pkl"%(i+1),"wb"))
    #indices = list(range(0,len(env_list)+1,int(len(env_list)/9)))
    for i in reversed(range(arglist.pos,arglist.end)):
        reward_list, _, state_list = multi_train(env_list[indices[i]:indices[i+1]])
        pickle.dump(reward_list,open("%d/%s/results%.1f/%d/rewards_list_%d.pkl"%(arglist.um,arglist.ty,dicts[arglist.ty],7,i),"wb"))
        pickle.dump(state_list,open("%d/%s/results%.1f/%d/state_list_%d.pkl"%(arglist.um,arglist.ty,dicts[arglist.ty],7,i),"wb"))