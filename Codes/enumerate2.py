...
def parse_args():
    ...
    parser.add_argument('--estep', metavar='N', type=int, default=3,
                    help='number of enumeration steps')
    ...
    parser.add_argument("--drl", metavar='dqn/ppo', type=str, default='ppo',
                    help='drl algorithm')
    ...
    def step(self):
        ...
        for s1 in self.hb_list:
            ...
            if (s1.ID in self.inside_attackers and self.rng1.random() < self.pida) or (s1.ID in self.oa_targets and self.rng1.random() < self.peda):
                temp_data = self.rng1.choice(self.landmarks).data
                #print("(data modified step,sensor id):(%d,%d)" % (self.steps,s1.ID))
            else:
                temp_data = s1.data
                temp_data[4] = s1.bl
            for agent in self.agents:
...
def train_all(env_list=None):
    ...
    state_list = []
    for env in env_list:
        for act1 in range(action_dim):
            for act2 in range(action_dim):
                temp_env = deepcopy(env)
                state, reward, done = temp_env.step([act1,act2])
                state_list.append(state)
                next_env_list.append(temp_env)
                reward_list.append(reward)
    return reward_list, next_env_list, state_list
...
def multi_train(envs):
    pool = Pool()
    #print("Separated results:")
    inputs = [[env] for env in envs]
    #print(len(inputs))
    results = pool.starmap(train, inputs)
    pool.close()
    pool.join()
    results = [list(itertools.chain.from_iterable([results[i][j] for i in range(len(results))])) for j in range(3)]
    return results

class tree_state(NodeMixin):  # Add Node feature
    def __init__(self, past_actions, reward, parent=None, children=None):
        super(tree_state, self).__init__()
...
def add_layer(ri,pre_actions0):
    next_pre_actions = []
    for groups in range(int(len(ri)/9)):
        current_action = 0
        for child in ri[groups*9:(groups+1)*9]:
            temp_action = np.append(pre_actions0[groups],current_action)
            next_pre_actions.append(temp_action)
            cur_int = actions_to_str(temp_action)
            cur = 's_' + cur_int
            pre = 's_' + actions_to_str(pre_actions0[groups])
            exec("%s = tree_state(past_actions='%s', reward=%f, parent=%s)" % (cur,cur_int,child[0],pre),globals())
            current_action += 1
    return next_pre_actions
...
def get_next_layer(node):
    try:
        return link_list(node.reward,[get_next_layer(i) for i in node.children])
    except IndexError:
        return link_list(node.reward,None)
...
if __name__ == "__main__":
    start = time.time()
    temp = pickle.load(open("actions/a_%s_%d_%s_%.1f.pkl"%(arglist.drl,arglist.um,arglist.adv,arglist.padv),"rb"))
    actions_list = []
    actions_set = []
    env_indices = []
    for i in range(len(temp)):
        actions = tuple(temp[i])
        actions_list.append(actions)
        flag = True
        for j in range(len(actions_set)):
            if actions_set[j] == actions:
                env_indices[j].append(i)
                flag = False
                break
        if flag:
    ...
    directory = 'e2/%s/'%(arglist.drl)
    if not os.path.exists(directory):
        os.makedirs(directory)

    pickle.dump(env_indices, open("e2/%s/ref_%d_%s_%.1f.pkl"%(arglist.drl,arglist.um,arglist.adv,arglist.padv),"wb"))
    all_actions = [[[actions[i]//3, actions[i]%3] for i in range(7)] for actions in actions_set]
    pickle.dump(all_actions,open("e2/%s/a_%d_%s_%.1f.pkl"%(arglist.drl,arglist.um,arglist.adv,arglist.padv),"wb"))

    #dist = [len(env_index) for env_index in env_indices]
    #print(dist)
    #print(len(actions_list),len(actions_set),len(env_indices),sum(dist))
    ...
    for actions in actions_set:
        ...
        length = 10
        sf = Scenario(arglist)
        env0 = ENV_test(length, sf, nagents)        
        state = env0.reset()
        for i in range(7):
            act1 = actions[i] // 3
            act2 = actions[i] % 3
            state, _, _ = env0.step([act1,act2])
        env_list = [env0]
        for i in range(steps):
            print("Sub_progress:%d/%d"%(i+1,steps))
            reward_list, env_list, state_list = multi_train(env_list)
            r_directory = 'e2/%s/r_%d_%s_%.1f/'%(arglist.drl,arglist.um,arglist.adv,arglist.padv)
            if not os.path.exists(r_directory):
                os.makedirs(r_directory)
            s_directory = 'e2/%s/s_%d_%s_%.1f/%d/'%(arglist.drl,arglist.um,arglist.adv,arglist.padv,count)
            if not os.path.exists(s_directory):
                os.makedirs(s_directory)
            state_list = [[int(states[j][i]) for j in range(2)] for states in state_list]
            pickle.dump([state_list[j][0] for j in range(len(state_list))],open(s_directory+'s%d%d.pkl'%(i,0),'wb'))
            pickle.dump([state_list[j][1] for j in range(len(state_list))],open(s_directory+'s%d%d.pkl'%(i,1),'wb'))
            if i == 0:
                for groups in range(int(len(reward_list)/9)):
                    temp_action = 0
                    for child in reward_list[groups*9:(groups+1)*9]:
                        #np.append(pre_actions[groups],temp_action)
                        pre_actions.append(temp_action)
                        cur_int = str(temp_action)
                        cur = 's_' + cur_int
                        pre = 'root'
                        exec("%s = tree_state(cur_int, child[0], parent=%s)" % (cur,pre))
                        temp_action += 1
            else:
                pre_actions = add_layer(reward_list,pre_actions)
        reward_tree = get_next_layer(root)
        pickle.dump(reward_tree,open(r_directory+"r_%d.pkl"%(count),"wb"))
        count += 1
        print("Progress:%d/%d"%(count,N))
        end = time.time()
        print("Time spent:%.3f"%(end-start))